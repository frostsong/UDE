...
def save_seed(obj):
    ...
    if isinstance(obj, dict):
        for key,values in obj:
            res.setdefault(key,[]).append(values)
        for value in obj.values():
            res.extend(save_seed(value))
    elif isinstance(obj, list):
        for item in obj:
            res.extend(save_seed(item))
    return res
...
def privacy_dependence_analyze(key, match_res):
    ...
    dependencies = myutil.parse_dependency(key)
...
def match_privacy_in_word_sequence(field):
    ...
    words = myutil.to_word_sequence_lower(field)
...
if __name__ == "__main__":
    test_case = {
        ...
        "byUserId": [
            "5c075f41f526ab6c26d2a18d",
            "6183980c144e306a7834507f",
            "618c88dd144e30d1c33df660"
        ]
    }
    ...
    # print(to_word_sequence_lower("paging_num"))
